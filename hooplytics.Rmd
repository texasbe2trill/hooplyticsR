...
players <- c("Klay Thompson", "Anthony Edwards", "Austin Reaves")
...
# Function to train Random Forest models for each category
...
```{r echo=FALSE, results='asis'}
# Function to evaluate models and make fantasy decisions based on over or under
make_fantasy_decisions_by_model <- function(predictions, test_data, player_thresholds = NULL, player_5_game_averages = NULL, confidence_margin = 0.1, weight_5_game_avg = 0.7) {
  ...
  for (i in 1:nrow(test_data)) {
    ...
    for (model_name in names(predictions)) {
      ...
      # Calculate a threshold based on player-specific thresholds or model predictions
      threshold <- if (!is.null(player_thresholds) && 
                       player_name %in% names(player_thresholds) && 
                       model_name %in% names(player_thresholds[[player_name]])) {
        player_thresholds[[player_name]][[model_name]]
      } else {
        mean(predictions[[model_name]], na.rm = TRUE)  # Default to mean of all predictions
      }

      # Incorporate 5-game average with additional weight if available
      if (!is.null(player_5_game_averages) && 
        ...
        # Weighted average between model threshold and 5-game average
        weighted_threshold <- (weight_5_game_avg * five_game_average) + ((1 - weight_5_game_avg) * threshold)
      } else {
        five_game_average <- NA  # Default to NA if no 5-game average is available
        weighted_threshold <- threshold  # If no 5-game average, use the threshold as is
      }

      # Decision based on prediction above or below weighted threshold and margin
      adjusted_threshold <- weighted_threshold * (1 + confidence_margin)  # Adjust threshold based on confidence margin

      # Ensure the threshold is used to make the decision
      ...
      all_predictions[[model_name]] <- list(
        ...
        threshold = round(weighted_threshold, 2),  # Store the calculated weighted threshold
        decision = decision,
        five_game_average = round(five_game_average, 2)  # Include the 5-game average
      )
    }
  ...
  for (player_name in names(results_by_player)) {
    ...
    # Format the player's results
    player_result <- glue::glue(
      "### **Player: {result$player_name}**\n",
      "**Best Prediction**: {result$best_prediction} | Model: {result$best_model} | Decision: {result$best_decision}\n\n",
      "<u>All Predictions:</u><br>"  # Add HTML <br> for line breaks
    )
    
    # Append all individual model predictions for the player
    for (prediction in result$all_predictions) {
      player_result <- glue::glue(
        player_result,
        "Model: **{prediction$model}** | Prediction: {prediction$prediction} | Threshold: {prediction$threshold} | 5-Game Avg: {prediction$five_game_average} | Decision: {prediction$decision}<br>"
      )
    }
    
    # Use `cat` with `asis` for R Markdown compatibility to print results
    cat(player_result, "\n\n", sep = "")
  }
}

# Example Player-Specific Thresholds
player_thresholds <- list(
...
# Example 5-game averages
player_5_game_averages <- list(
  
  "Austin Reaves" = list(
    points_model = round(mean(c(22, 45, 23, 20, 27)), 2)
  ),
  "Anthony Edwards" = list(
    points_model = round(mean(c(36, 21, 49, 41, 44)), 2)
  ),
  "Klay Thompson" = list(
    points_model = round(mean(c(2, 12, 25, 13, 19)), 2)
  )
)

# Make fantasy decisions based on the predictions and 5-game averages
make_fantasy_decisions_by_model(predictions, test_data, player_thresholds, player_5_game_averages)

# Write the predictions to a text file
writeLines(capture.output(make_fantasy_decisions_by_model(predictions, test_data, player_thresholds)), "data/predictions.txt")
```
