...
# Example usage:
# Assuming 'player_stats_df' is your data frame with the required columns.
...
```{r combined_plot, eval=FALSE, fig.height=10, fig.width=16, include=FALSE, out.width="100%"}
# # Arrange the plots in 2 columns
# combined_plot_1 <- (p1 + p2) / (p3 + p4) + 
#   plot_layout(ncol = 2)
#   #plot_annotation(title = "Player Distributions Summary")
# 
# combined_plot_2 <- (p5 + p6) / (p7 + p8) +
#   plot_layout(ncol = 2)
...
player_tables <- lapply(results, function(player) {
  ...
  df <- do.call(rbind, lapply(player$all_predictions, function(pred) {
    data.frame(
      ...
      `5 Game Avg` = pred$five_game_average,
      Decision = pred$decision,
      stringsAsFactors = FALSE
    )
  }))
  
  # Create a caption that includes the player's name and key details
...
# For each player's result, print a markdown subheading with the player's name,
# then print the table with the caption (which also includes the player's name).
